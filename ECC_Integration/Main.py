...
Secure Boot + ECDH + ASCON-128a + ECDSA
"""
...
class SecureEmbeddedSystem:
    ...
    def secure_boot(self, firmware_path, signing_private_key, signing_public_key):
        ...
        try:
            ...
            if is_valid:
                ...
                self.log_event("SECURE_BOOT", "SUCCESS", f"Hash: {hash_value.hex()[:16]}")  # type: ignore
            else:
                self.boot_status = False
    ...
    def encrypt_and_send(self, message, recipient_node):
        ...
        try:
            ...
            # Simulate receiving
            decrypted = recipient_node.secure_node.decrypt_message(
                nonce, ciphertext, tag
            )
            print(
                f"[{recipient_node.node_id}] Message received: {decrypted.decode('utf-8')}"
            )
            recipient_node.log_event(
                "MESSAGE_RECEIVED", "SUCCESS", f"Length: {len(decrypted)}"
            )
    ...
    def log_event(self, event_type, status, details):
        ...
        log_entry = (
            f"{timestamp} | {self.node_id} | {event_type} | {status} | {details}\n"
        )
...
def main_demo():
    ...
    print("Secure Boot | ECDH | ASCON-128a | ECDSA")
    print("=" * 70)
