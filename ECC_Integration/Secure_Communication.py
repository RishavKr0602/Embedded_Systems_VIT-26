...
Integrates: ECDH + HKDF + AES-256-GCM + ECDSA
"""
...
class SecureCommunicationNode:
    ...
    def encrypt_message(self, plaintext):
        """
        Encrypt message using AES-256-GCM with current session key
        """
        if not self.session_key:
            raise ValueError("No session key established")
        
        aes = AESGCMEncryption(self.session_key)
        nonce, ciphertext, tag = aes.encrypt(plaintext.encode('utf-8'))
...
def demo_complete_secure_pipeline():
    ...
    print("ECC + ECDH + HKDF + AES-256-GCM + ECDSA")
    print("="*70)
    ...
    print(f"Session Key A: {session_key_a.hex()[:32]}...")
    print(f"Session Key B: {session_key_b.hex()[:32]}...")
